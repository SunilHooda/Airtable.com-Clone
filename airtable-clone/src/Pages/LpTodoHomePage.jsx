...
const LpTodoHomePage = () => {
  ...
  return (
    <Box width="100%" paddingTop="1rem" border="1px solid rgba(0,0,0,0.3)">
      <Flex direction="row" justifyContent="space-around">
        {/* PENDING TASKS  */}
        <Box width="32%" height="auto" border="1px solid rgba(0,0,0,0.3)">
          ...
          <Box>
            {tasks.length > 0 &&
              tasks
                .filter(
                  (item) =>
                    item.task_status === "todo" && item.userID === "userID"
                )
          ...
          <Box>
            {tasks.length > 0 &&
              tasks
                .filter(
                  (item) =>
                    item.task_status === "progress" && item.userID === "userID"
                )
                .filter(filterByParamTags)
                .map((item) => {
                  return (
          ...
          <Box>
            {tasks.length > 0 &&
              tasks
                .filter(
                  (item) =>
                    item.task_status === "done" && item.userID === "userID"
                )
                .filter(filterByParamTags)
                .map((item) => {
                  return (
